...
#if defined(SPEC)
// the standard reserves std::abs  -- illegal to redefine
// Suggestion from Andras to just rename it to abs64
static inline int64 abs64(int64 x) { return x >= 0 ? x : -x; }
#else
...
#if defined(SPEC)
    int64 val = abs64((long)this->intVal);
#else
