...
void cDynamicExpression::Elem::operator=(const Elem& other)
{
    ...
    if (type==STR)
        ...
        constant_subexpression = (cExpression *) constant_subexpression->dup();
}
...
#define CMP(x) (x==other.x ? 0 : x<other.x ? -1 : 1)
      ...
      case CONSTSUBEXPR: return constant_subexpression->compare(other.constant_subexpression);
...
#if defined(SPEC_NEED_TRUNC)
inline double trunc(double x)
{
   return x < 0.0 ? ceil(x) : floor(x);
}
#endif /* SPEC_NEED_TRUNC */
    ...
    try
    {
        ...
        for (int i = 0; i < size; i++)
        {
           ...
           switch (e.type)
           {
               ...
               case Elem::CONSTSUBEXPR:
                 strstk[++tos] = std::string("const(")+e.constant_subexpression->str()+")";
                 break;
               case Elem::OP:
                 if (e.op==NEG || e.op==NOT || e.op==BIN_NOT)
                 {
